...
const loadModel = async () => {
  try { // Объявление асинхронной функции loadModel
    const model = await tf.loadLayersModel('model/model.json');
    // Загрузка модели с помощью TensorFlow.js и ожидание завершения операции
    console.log('Модель TensorFlow.js загружена', model);
    // Вывод сообщения в консоль о успешной загрузке модели
    return model;
  } catch (error) {  // Возвращение загруженной модели
    console.error('Ошибка загрузки модели TensorFlow.js:', error);
    // Обработка ошибки, если загрузка модели не удалась
...
const handleImageUpload = (event) => {
  // Объявление функции которая будет вызываться при загрузке изображения
  const selectedFiles = event.target.files;
  // Получение списка выбранных файлов из события
  if (selectedFiles) {
    for (let i = 0; i < selectedFiles.length; i++) {
      const file = selectedFiles[i];
      // Проверка, что файлы были выбраны дальнейшая итерация по каждому выбранному файлу,
      // получение текущего файла
      images.value.push(file);
    }
    predictAnimal(); // Добавление файла в массив и вызов функции predictAnimal
  } // для предсказания животного на изображении
};
  ...
  if (model.value && images.value.length > 0) {
    try {
      const predictionPromises = images.value.map(async (image) => {
        ...
        await new Promise((resolve) => { // Ожидание загрузки изображения
          img.onload = resolve;
        });
        canvas.width = img.width; // Установка размеров canvas под размеры изображения
        canvas.height = img.height;
        ctx.drawImage(img, 0, 0);         // Отрисовка изображения на canvas
